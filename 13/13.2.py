class Counter:
    ...
    def __init__(self, current=1, min_value=0, max_value=10):
        ...
        if not (min_value <= current <= max_value):
            raise ValueError("Поточне значення поза допустимими межами")
...
assert counter.get_current() == 10, 'Test_1'
...
assert counter.get_current() == 10, 'Test_2'
...
assert counter.get_current() == 7, 'Test_3'
...
assert counter.get_current() == 7, 'Test_4'